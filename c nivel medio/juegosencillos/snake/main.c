...
int main()
{
    int tam;
    char campo[V][H];
    inicio(&tam,campo);//iniciar todos los elementros
   loop(campo,tam);
   printf("                            Has perdido\n");
   system("pause");
    return 0;
...
//Metera todos los datos en la matriz camp
void Intro_Datos(char campo[V][H],int tam){
 int i;
    for(i=1;i<tam;i++){
...
//comprobar si nos hemos tragado la fruta
...
if(*muerto==0){
  if(kbhit()==1){
    key=getch();
    if(key=='k'&&snake[0].ModY!=-1){
snake[0].ModX=0;
snake[0].ModY=1;
    }
    if(key=='i'&&snake[0].ModY!=1){
snake[0].ModX=0;
snake[0].ModY=-1;
    }
    if(key=='j'&&snake[0].ModX!=1){
snake[0].ModX=-1;
snake[0].ModY=0;
    }
    if(key=='l'&&snake[0].ModX!=-1){
snake[0].ModX=1;
snake[0].ModY=0;
    }
  }
...
void Intro_Datos2(char campo[V][H],int tam){
int i;
//crear el moviemiento de la serpeinte
